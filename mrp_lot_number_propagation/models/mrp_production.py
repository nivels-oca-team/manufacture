...
class MrpProduction(models.Model):
    ...
    def _create_and_assign_propagated_lot_number(self):
        for order in self:
            ...
            if finish_moves: # nivels change because quantity_done field no longer exists
    ...
    def write(self, vals):
        for order in self:
            if (
                ...
                and vals.get("lot_producing_id") != order.lot_producing_id.id # nivels change to allow print labels button in shop floor
            ):
                raise UserError(
                    _(
